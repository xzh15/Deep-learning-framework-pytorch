...
案例：
    演示张量的基本创建方式
张量：
    PyTorch框架属于最常用的深度学习框架， 无论是后续学习ANN（人工神经网络），还是CNN（卷积神经网络），RNN（循环神经网络）
    底层在处理数据时，都是使用的 张量 来处理的。

    张量 --> 存储同一类型元素的容器，且元素必须是 数值 才可以。
...
def dm03():
    ...
    data = np.random.randint(0, 10, (2, 3))
    t4 = torch.FloatTensor(data)  #默认数据类型float32
    print(f't4: {t4}, type: {type(t4)}')
